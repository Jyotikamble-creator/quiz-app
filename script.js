...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchQuestions() {
        try {
            ...
            userAnswers = Array(questions.length).fill(null); // Track user's choices
        } catch (error) {
            console.error("Error fetching questions:", error);
        }
    }
    ...
    function showQuestion() {
        ...
        currentQuestion.choices.forEach(choice => {
            const li = document.createElement("li");
            li.textContent = choice;
            li.classList.add("cursor-pointer", "p-2", "m-2", "bg-white", "rounded", "hover:bg-gray-200", "transition");

            if (userAnswers[currentQuestionIndex] === choice) {
                li.classList.add(choice === currentQuestion.answer ? "bg-green-300" : "bg-red-300");
            }
    ...
    function navigateQuestion(direction) {
        ...
        // If finished
        if (currentQuestionIndex === questions.length && direction === 1) {
            showResult();
        }
    }
    ...
    function showResult() {
        ...
        scoreDisplay.textContent = `Final Score: ${finalScore} out of ${questions.length}`;
    }
